...
namespace TheMarkedMen
{
    public static class MarkedIdeologyUtility
    {
        ...
        private static readonly IdeoIconDef CachedFixedIcon = DefDatabase<IdeoIconDef>.GetNamedSilentFail(FixedIconDefName);
        private static readonly ColorDef CachedFixedColor = DefDatabase<ColorDef>.GetNamedSilentFail(FixedColorDefName);
        private static readonly StyleCategoryDef CachedFallbackStyleCategory = DefDatabase<StyleCategoryDef>.GetNamedSilentFail(FallbackStyleCategoryDefName);
        private static readonly MemeDef[] CachedFallbackMemes = InitFallbackMemes();

        private static MemeDef[] InitFallbackMemes()
        {
            var list = new List<MemeDef>(FallbackMemeDefNames.Length);
            for (int i = 0; i < FallbackMemeDefNames.Length; i++)
            {
                MemeDef meme = DefDatabase<MemeDef>.GetNamedSilentFail(FallbackMemeDefNames[i]);
                if (meme != null) list.Add(meme);
            }
            return list.ToArray();
        }
        ...
        {
            IdeoIconDef iconDef = CachedFixedIcon;
            ColorDef colorDef = CachedFixedColor;
        ...
        private static List<MemeDef> BuildTargetMemes(FactionDef factionDef)
        {
            ...
            if (targetMemes.Count == 0)
            {
                for (int i = 0; i < CachedFallbackMemes.Length; i++)
                {
                    MemeDef meme = CachedFallbackMemes[i];
        ...
        private static List<StyleCategoryDef> BuildTargetStyles(FactionDef factionDef)
        {
            ...
            if (targetStyles.Count == 0)
            {
                if (CachedFallbackStyleCategory != null)
                {
                    targetStyles.Add(CachedFallbackStyleCategory);
                }
            }
